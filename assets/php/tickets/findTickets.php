...
function findTickets($userID, $status) {
    ...
    } else if($status == 'terceiros'){
        $sqlTicket .= "AND status = 3 ORDER BY glpi_tickets.id DESC";
    } else if($status == 'usuario') {
        $sqlTicket .= "AND status = 4 ORDER BY glpi_tickets.id DESC";
    } else if($status == 'solucionado') {
        $sqlTicket .= "AND status = 5 ORDER BY glpi_tickets.id DESC";
    } else {
        $sqlTicket .= "AND status != 6 ORDER BY glpi_tickets.id DESC";
    }
