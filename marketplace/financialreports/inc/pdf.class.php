   ...
   function display_table($total, $items, $deviceType, $disposal = 0) {
      ...
      if ($total != 0) {
         ...
         foreach ($items as $data) {
            ...
            $this->SetTextBlue();
            $this->CellLineTable(false, 50,$dbu->formatUserName($data["ITEM_4_3"], $data["ITEM_4"], $data["ITEM_4_2"], $data["ITEM_4_4"], 0));
            $this->SetTextBlack();
            if ($disposal != 1) {
            ...
            if ($disposal == 1) {
               ...
               $this->SetTextBlack();
               $this->CellLineTable(false, 35, Html::convDate($data["ITEM_10"]), 1, 'C', 1);
